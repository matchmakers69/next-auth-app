...
const PostList = async ({ fetchData }: PostListProps) => {
  ...
  return (
    <ul className="flex w-full flex-col gap-6">
      {posts.map((post) => (
        <li
          className="flex items-center overflow-hidden rounded px-[1rem] py-0 odd:bg-[hsla(0,0%,100%,0.05)] even:bg-[hsla(0,0%,100%,0.08)]"
          key={post.id}
        >
          <Link
            className="p-section flex h-full w-full flex-col justify-between text-text-grey transition-colors duration-200 hover:text-light-blue"
